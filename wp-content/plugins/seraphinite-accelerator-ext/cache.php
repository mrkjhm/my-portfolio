...
function _OutImage( $siteId, $settGlob, $sett, $file, $nonce, $aiFileId = null )
{
	...
	if( ($sett[ 'hdrTrace' ]??null) )
		@header( 'X-Seraph-Accel-Cache: 2.27.32; state=' . ( $mimeType == Fs::MimeTypeDef ? 'original' : 'preoptimized' ) . '; redir=alt;' . ( $aSkip ? ( '; skip=' . @json_encode( $aSkip ) ) : '' ) . ( $mimeType != Fs::MimeTypeDef ? ( '; sizeOrig=' . @filesize( $fileOrig ) ) : '' ) );
	...
	if( $isCip === false )
	{
		if( $seraph_accel_g_prepPrms === null )
		{
			...
			if( $seraph_accel_g_simpCacheMode !== null )
			{
				...
				return( ( !($settCache[ 'updByTimeout' ]??null) || CacheSetCurRequestToPrepareAsync( $contProc ? $seraph_accel_g_siteId : null, $seraph_accel_g_lazyInvTmp, $bgEnabled ) ) ? Gen::S_OK : Gen::S_FALSE );
...
function _ProcessOutHdrTrace( $sett, $bHdr, $bLog, $state, $data = null, $dscFile = null )
{
	...
	if( $bHdr )
		@header( 'X-Seraph-Accel-Cache: 2.27.32;' . $debugInfo );
...
function GetCacheViewId( $ctxCache, $settCache, $userAgent, $path, $pathOrig, &$args )
{
	...
	if( ($settCache[ 'normAgent' ]??null) )
	{
		...
		$_SERVER[ 'HTTP_USER_AGENT' ] = 'Mozilla/99999.9 AppleWebKit/9999999.99 (KHTML, like Gecko) Chrome/999999.0.9999.99 Safari/9999999.99 seraph-accel-Agent/2.27.32';
	}
