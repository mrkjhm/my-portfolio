...
function OnAsyncTask_CacheProcessItem( $args )
{
	...
	if( $itemType == 0 )
	{
		...
		if( ($settGlobal[ 'asyncSmpOpt' ]??null) )
		{
			...
			ProcessQueueItemCtx::MakeRequest( $asyncMode, 'GET', Net::UrlAddArgs( Plugin::AsyncTaskPushGetUrlEx( Wp::GetSiteWpRootUrl( OnAsyncTasksGetPushUrlFile( true ) ), 'O', $tmStamp ), array( 'nonce' => hash_hmac( 'md5', Plugin::AsyncTaskPushGetTimerun( $tmStamp ), GetSalt() ) ) ) );
			return;
		}
